...
- **Series chip gets sizing priority over decoration, not the other way round, when space is tight next to the fixed pill** (`LiveCarousel.tsx`, v1.0.71): after the pill went fixed-width (176px) and the row went `flex-nowrap` (above), the chip's leftover space (~167px of its ~222px slot, after its own padding/gap/icons) was narrower than the real `seriesStatus` strings in the dataset (measured 168-179px at the sizes used here — see `lib/mockData.ts`'s 3 current values), so `truncate` was firing on the everyday case, not just a hypothetical long-tail one. Fixed by trimming the chip's own chrome rather than touching the pill: dropped the purely-decorative trailing chevron (the border-color hover already signals tappability without it), tightened `px-3`→`px-2.5` and the icon-to-text `gap-1.5`→`gap-1`, and reduced the label's own type size `text-[11px]`→`text-[9.5px]` (the pill's 11px stays untouched — this is deliberately a smaller, secondary-info size, not a matched one). At these sizes the longest current real string (`"Series level 0-0 · 5-match T20I series"`, 179px) still clears the available ~187px with a real ~8px margin, measured live, not assumed. `truncate` + `min-w-0` stay on the label span as the genuine last-resort for a future materially longer series description — that's the only case this should ever visibly clip.
