...
| `orange` | `#FF6B35` | Mid-tier attention — ESPN data-source badge, ball-speed mid-band (`DeliveryCard`/`BallGIF`), elevated-but-not-critical RRR in `ScoreBar`. A "notice this, but it's not alarming" tier, distinct from `wicket` red. (The v1.0.160 `PitchReportCard` redesign removed the old pace/dew/expected-score sliders that used to share this tier — see the `PitchReportCard` note under §4.) |
...
| `follow` | `#7C3AED` | Filter/personalization only — see §4. As of v1.0.115, narrower than it once was: `FollowSheet`'s own selection-state UI (checkbox fill, category count badges, "Update" button) moved to `cyan` to match the platform's single active/selected accent; `follow` now covers only the bottom-nav Filter icon/label and the homepage "FOR YOU" nudge. Deliberately a different violet from `six`'s purple; do not blur the two. |
...
| `spin` | `#A855F7` | Ball spin-direction / delivery-type indicator — `BallGIF`, `MiniBallGIF`, `DeliveryCard`, `MomentStoryCard`. Carved out of `six` in v1.0.67 (same hex, new name) — a spin delivery isn't a six-run outcome (a fast bowler's slower ball can also land in the spin-colored bucket while scoring zero runs). (`PitchReportCard`'s old "Spin-friendly" slider used this token too, but the v1.0.160 redesign replaced every slider with colorless `StatCell` boxes — `PitchReportCard` no longer imports this token at all.) |
...
|---|---|---|---|
| `follow` (`#7C3AED`) | Violet 600 | As of v1.0.115: bottom-nav Filter icon/label while the sheet is open, and the homepage "FOR YOU" label + its card's colored border logic route. (`FollowSheet`'s own checkbox fill, category count badges, and "Update" button moved to `cyan` in v1.0.115 — see below — so the sheet reads as the same platform accent as everywhere else, not a separate violet feature.) | Any ball outcome, any team's brand color, any other "selected/active" state elsewhere in the app. It was deliberately created as its *own* violet specifically so it would never be confused with `six`'s purple (`#A855F7`) — see the inline comment already in `tailwind.config.ts`. If a new feature needs a "this is selected" accent, it should reuse the platform `cyan` accent (as `FollowSheet` now does) or get its own token, not reuse either purple. |

**Raw-hex sites (v1.0.67):** SVG `stroke`/`fill` attributes and inline `style` objects can't use a Tailwind class at all — they need a literal hex string. `lib/tokens.ts` exports `LIVE`, `NEGATIVE`, `BOUNDARY`, `SPECIAL`, `SPIN`, `SLOW_PACE` as plain string constants mirroring the Tailwind tokens above, for exactly those sites (see `BallGIF.tsx`, `MiniBallGIF.tsx`, `MomentStoryCard.tsx`, `DeliveryCard.tsx` for real usage — `PitchReportCard.tsx` was on this list before the v1.0.160 redesign extracted its stat row into the colorless shared `StatCell` component; it no longer imports `lib/tokens.ts` at all). There's no build-time link between this file and `tailwind.config.ts` — keep them in sync by hand if either changes.
