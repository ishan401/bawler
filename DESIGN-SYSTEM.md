...
- **Table-shortcut pill + series-info chip prefer one row, but the chip never changes its own styling to force that** (`LiveCarousel.tsx`, the `flex items-center gap-2 flex-wrap` container just below the hero card, v1.0.72): the fixed-width `TABLE` pill (176px, above) and the series-status chip (`"Series level 1-1 · 3-match Test series"` or equivalent, opens the series schedule) sit inline whenever they both fit. The chip's own font size (`text-[11px]`, matching the pill and every other homepage chip/label), padding (`px-3 py-1.5`), icon-to-text gap (`gap-1.5`), and both icons are fixed and never shrink to fit — this was tried once (v1.0.71, `text-[9.5px]` + tightened chrome) and reverted, because auto-shrinking a chip's type size to solve a space problem reads as a bug, not a feature, and breaks consistency with every other same-size chip on the page. The container's `flex-wrap` is the actual pressure-release valve: when the pill's fixed width leaves less room than the chip's full-size natural width — the real case for the current Test-match `seriesStatus` string, which needs ~263px against ~222px available — the chip wraps to its own line below the pill, still rendered at full size, rather than shrinking or truncating. `truncate` + `min-w-0` remain on the label as a genuine last resort, only for a future description too long to read comfortably even on its own full-width line. **If a future pass wants both elements on one row for a long string, the fix is more available width or a different pairing — never a smaller font or tighter chrome on this chip specifically.**
