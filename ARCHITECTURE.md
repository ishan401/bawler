...
## Worked example — deferring a non-user-initiated state update so it can never land mid-tap: `lib/pointerGuard.ts` — v1.0.151

**The gap this closes.** `components/MatchView.tsx` runs a demo-only `setInterval` (gated by `shouldRunMockSimulationTicker`, only for matches explicitly flagged `isMockSimulation: true`) that advances `liveBallIdx` every `BALL_DWELL_MS` while viewing a mock-simulated live match. That `setState` flows through `truncatedMatch`'s `useMemo` and re-derives `battingCard`/`bowlingCard` fresh on every tick, which mutates the DOM of whatever's currently live — a not-out batter's stat line, the current bowler's figures, `BallGIF`'s conditionally-rendered share button, and similar elements. A tap landing in that same instant could be dropped by the browser before any click handler ever ran it — confirmed via direct capture-phase instrumentation showing zero DOM events fired for the dropped click, i.e. a browser-level event-delivery race against a real DOM mutation, not an application `preventDefault`/`stopPropagation` bug or a malformed `href`.

**Two independent fixes, not one.** (1) Reduce unnecessary DOM churn on interactive elements themselves where possible — e.g. `components/Scorecard.tsx`'s `PlayerNameLink` wrapped in `React.memo` so a batter's name/link never re-renders from a tick that only changed their ticking stats. (2) `lib/pointerGuard.ts`'s `runGuarded()` — a safety net that holds even where (1) can't fully apply (`BallGIF`'s share button must legitimately mount/unmount based on whether the CURRENT ball is a "big moment," so no amount of memoization makes its presence stable): it tracks whether a pointer/touch gesture is currently in progress anywhere on the page via passive `pointerdown`/`pointerup`/`pointercancel` (+ `touchstart`/`touchend`/`touchcancel` fallback) listeners, and defers the callback passed to it until the gesture ends if one is in progress, running it immediately otherwise. `MatchView.tsx`'s ticker now calls `runGuarded(() => setLiveBallIdx(...))` instead of calling `setLiveBallIdx` directly — the state update, and therefore the DOM mutation it causes, can never land mid-tap.

**Why a module-level queue, not component state.** `queuedCallbacks` is a plain `Set` at module scope, not `useState`/`useRef` inside a component — `runGuarded()` is meant to be callable from any future `setInterval`-driven, non-user-initiated update anywhere in the app, not just `MatchView`'s ticker, so it deliberately has no dependency on any one component's lifecycle. A callback reference still queued from an earlier tick (a pointer held down across multiple tick periods, e.g. a long-press) coalesces into that single pending run on `flushQueue()` rather than piling up duplicate calls, since `Set.add()` on the same function reference is a no-op.

**Verification recipe** (full detail in DECISIONS-LOG.md, "click swallowed by the live-simulation ticker"): direct capture-phase click-collision test against every affected interactive element (batter name links, current-bowler figures, `BallGIF`'s share button) confirmed zero dropped clicks with the guard in place, versus reproducible drops without it; `tsc --noEmit`/`npm run build` clean.

## Worked example — extending an existing id-or-name matching pattern to a new caller instead of writing new matching logic: `MiniInsightsBar.tsx`'s bowler chip — v1.0.164

**The gap this closes.** The top chip strip's bowler chip (`components/MiniInsightsBar.tsx`) looked up the current bowler's live figures in `live.bowlingCard` via fragile `.includes()` substring matching (`b.playerName.includes(currentBowlerName) || currentBowlerName.includes(b.playerName)`), with no fallback if that lookup missed. On `ipl2026-m37-kkrvmi`, the ball data used short bowler names ("J Bumrah") while `bowlingCard` used a different format ("Jasprit Bumrah", "P. Krishna" with a period) — neither string was a substring of the other, so the lookup always failed and the chip silently disappeared with no error, at every point in the match where those bowlers were on.

**Why this reused `samePlayer()` rather than writing new matching logic.** `lib/matchStatus.ts` already has `samePlayer(id, name, entryId, entryName)` — the id-or-name union predicate (`id === entryId || name === entryName`) built for exactly this class of problem and already used by `deriveBattingCardFromBalls`/`deriveBowlingCardFromBalls` for every other ball-to-card join. The batter chips in this same component already had a working fallback pattern (recompute from `live.balls` when the card lookup misses) that the bowler chip never got an equivalent of. Rather than inventing a third matching scheme, the fix (1) swapped the bowler chip's lookup to `samePlayer(currentBowlerId ?? "", currentBowlerName, b.playerId, b.playerName)`, matching by id first and falling back to exact name only when no id is present, and (2) added a balls-derived fallback mirroring the batter chips': when the `bowlingCard` lookup still misses, `deriveBowlingCardFromBalls(live.balls, [], match.format)` is called directly (reusing the same wickets/runs-conceded/overs math every other consumer already relies on, not reimplemented inline) and the same `samePlayer()` predicate searches its output instead.

**The actual data fix, kept separate from the code fix.** Robust matching alone doesn't help if the two sides being compared don't even agree on id OR name — `ipl2026-m37-kkrvmi`'s `bowlingCard` `playerName` values were normalized to the same short form its own `playerId`s and the ball data already used (e.g. "Pat Cummins" → "P Cummins", "Hardik Pandya" → "H Pandya", 12 values across both innings). This shipped as an incremental commit distinct from the `MiniInsightsBar.tsx` code change — the code fix makes future name-format drift non-fatal (the chip degrades to the balls-derived fallback instead of disappearing), while the data fix removes the one drift that had already happened.

**Verification recipe** (full detail in DECISIONS-LOG.md v1.0.164): bowler chip confirmed present on `ipl2026-m37-kkrvmi` at multiple points with correct wickets/runs/overs, cyan threshold (`wickets >= 2`) confirmed still triggering correctly now that the chip is reliably visible; the three previously-unaffected matches (`ind-aus-t20i-2026-m2-live`, `ind-eng-test-2026-d3-live`, `psl-2026-lah-kar-live`) re-checked for unchanged behavior; batter chip logic confirmed untouched via `git diff`; full platform regression pass; `tsc --noEmit`/`npm run build` clean.

## Worked example — one shared tab-switching hook instead of six independent copies: `lib/useTabSwitcher.ts` — v1.0.168

**The gap this closes.** A real, reproduced bug on match pages (DECISIONS-LOG.md, "tab-switch stale-content bleed") traced to `MatchView.tsx`'s Live/Score/Digest/Info switcher keeping the active-tab highlight and the actually-mounted content as two separate pieces of state, the second updated only inside a `setTimeout` — a gap that widens, not shrinks, under real main-thread load. A platform-wide audit for the same shape found five more independent copies of tab/segmented-view-switching state (`PageTransition.tsx`, `PlayerProfileView.tsx`, `app/schedule/page.tsx`, `FollowSheet.tsx`, `DigestTab.tsx`), none of which reset scroll position on a switch either, and one of which (`PageTransition.tsx`, wrapping every page via `app/layout.tsx`) had the identical timer-desync defect as `MatchView.tsx` itself.

**Why one hook, not six patches.** Six independent fixes would have fixed six instances of the bug as it exists today and guaranteed nothing about the seventh tab UI someone builds next month. `lib/useTabSwitcher.ts`'s `useTabSwitcher()` makes both defects structurally impossible for any caller: it exposes exactly one state value (`activeTab`) for both "what's highlighted" and "what's mounted" to read — there is no second, delayed copy that a `setTimeout` could leave behind — and its `switchTab()` resets scroll (window, or a passed `scrollContainerRef`) synchronously in the same call that updates state, before any timer could even be scheduled. A caller cannot reintroduce either defect without bypassing the hook entirely, and grepping for stray `setTimeout`-gated "rendered" state or hand-rolled `scrollTo`/`scrollTop = 0` outside this one file is now a meaningful regression check.

**Two shapes of caller, one shared reset.** Most tab UIs own their active-tab value as local state set by a click or swipe handler — `useTabSwitcher()` covers those directly. Two callers don't fit that shape: `PageTransition.tsx`'s active view is Next's own router `pathname`, and `DigestTab.tsx`'s active day/innings can silently advance to a new "latest" as live data arrives, with no click involved. Rather than have those two hand-roll their own `scrollTo` effect, `useScrollResetOnChange(key, scrollContainerRef?)` is a companion hook exposing the identical reset logic keyed off any string — `pathname` for one caller, a synthesized `` `digest-day-${activeDay}` ``/`` `digest-inn-${activeInnings}` `` key for the other — so the codebase has exactly one scroll-reset implementation total, not one per caller shape.

**The one legitimate bypass, kept narrow and named.** `restoreTab()` exists for exactly one case: silently correcting `activeTab` from a saved `sessionStorage` value right after mount (the hydration-safe pattern established in v1.0.89/v1.0.90 — render the same default on server and client's first pass, correct only in a post-mount effect). It skips the scroll reset and animation deliberately, since the user didn't click or swipe anything; calling it from an interaction handler would incorrectly yank scroll on a refresh. `MatchView.tsx`'s sessionStorage-tab-restore and nothing else in this codebase calls it.

**Verification recipe** (full detail and every per-surface result in DECISIONS-LOG.md): all six surfaces re-implemented against the shared hook, `tsc --noEmit`/`npm run build` clean after each; grep confirmed zero residual independent tab-switching state outside `lib/useTabSwitcher.ts`; live per-surface click/swipe/stress-test verification on the deployed app covering every `MatchView` tab pair in both directions, on live/finished/pre-match fixtures, plus an artificial ~900ms main-thread block re-run to confirm zero pill/content mismatch window (not shorter — zero).
